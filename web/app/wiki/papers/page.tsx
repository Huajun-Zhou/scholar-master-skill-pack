...
export default async function PapersPage() {
  ...
  const byYear: Record<number, PaperSummary[]> = {};
        ...
        {Object.entries(byYear)
          ...
          .map(([yearStr, yearPapers]) => (
            <div key={yearStr} className="mb-10">
              <h2 className="text-xl font-serif font-semibold mb-4 text-[var(--color-gold)] sticky top-0 bg-[var(--bg-base)] py-2 z-10">
                {yearStr}
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {yearPapers.map((paper) => (
                  <Link
                    key={paper.paper_id}
                    href={`/wiki/papers/${paper.paper_id}`}
                  ...
                  >
                    <h3 className="text-sm font-medium leading-snug mb-2 group-hover:text-[var(--color-gold)] transition-colors line-clamp-2">
                      {paper.title}
                    </h3>
                    <div className="flex flex-wrap items-center gap-2 text-[0.65rem] text-[var(--text-tertiary)]">
                      ...
                      {paper.evidence_count > 0 && (
                        <span className="px-1.5 py-0.5 rounded bg-[var(--color-evidence-a-bg)] text-[var(--color-evidence-a)] font-mono">
                          {paper.evidence_count} EVIDs
                        </span>
                      )}
                    </div>
                  </Link>
